...
class Concept(object):
    ...
    def __repr__(self):
        return "Concept(%r, %r)" % (self.language, self.text)

    def all_relations(self, types='All', limit=10):
        return [Assertion._from_cache(a) for a in
                all_relations(self.language, self.text, types, limit)]

    def fwd_relations(self, types='All', limit=10):
        return [Assertion._from_cache(a) for a in
                fwd_relations(self.language, self.text, types, limit)]

    def rev_relations(self, types='All', limit=10):
        return [Assertion._from_cache(a) for a in
                rev_relations(self.language, self.text, types, limit)]
    ...
    def update_cache(self):
        self._cache = get_json([self.language, 'concept', self.text])
...
def similar_concepts(list_of_concepts, lang=None):
    if hasattr(list_of_concepts[0], 'language'):
        lang = list_of_concepts[0].language
    elif lang is None:
        lang = 'en'

    def get_text(concept):
        if hasattr(concept, 'text'): return concept.text
        else: return concept
    parts = [get_text(c) for c in list_of_concepts]
    similarity = get_json([lang, 'concepts', 'near'] + parts)
    return [(lang, Concept(x['text']), x['score']) for x in similarity['similar']]
