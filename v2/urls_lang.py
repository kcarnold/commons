...
urlpatterns += patterns(
    ...
    url(r'^assertion/(?P<assertion_id>\d+)/$', 'assertion', name='assertion'),
    url(r'^add/', 'add_main', name='add_main'),
    url(r'^add_statement/(?P<frame_id>\d+)/(?P<text1>.+)/(?P<text2>.+)/$', 'add_statement', name='add_statement'),
...
urlpatterns += patterns(
    ...
    # (r'^features/predict/(?P<category>.*)/$', 'predict_features'),
    (r'^concepts/near/(?P<concept_name>.*)/$', 'similar_concepts'),
    (r'^features/predict/(?P<concept_name>.*)/$', 'predict_features')
)
